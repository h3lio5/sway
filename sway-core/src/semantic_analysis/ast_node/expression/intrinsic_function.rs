...
fn type_check_size_of_val(
    ...
    let return_type = type_engine.insert(TypeInfo::UnsignedInteger(IntegerBits::SixtyFour));
...
fn type_check_size_of_type(
    ...
    let initial_type_id = type_engine.insert(initial_type_info);
    let type_id = check!(
...
fn type_check_is_reference_type(
    ...
    ok(
        (intrinsic_function, type_engine.insert(TypeInfo::Boolean)),
        warnings,
        errors,
    )
}
...
fn type_check_get_storage_key(
    ...
    ok(
        (
            ...
            type_engine.insert(TypeInfo::B256),
        ),
        vec![],
        vec![],
    )
...
fn type_check_addr_of(
    ...
    let return_type = type_engine.insert(TypeInfo::RawUntypedPtr);
    ok((intrinsic_function, return_type), warnings, errors)
}
...
fn type_check_state_clear(
    ...
    let return_type = type_engine.insert(TypeInfo::Boolean);
    ok((intrinsic_function, return_type), warnings, errors)
}
...
fn type_check_log(
    ...
    let return_type = type_engine.insert(TypeInfo::Tuple(vec![]));
    ok((intrinsic_function, return_type), warnings, errors)
}
...
fn type_check_revert(
    ...
    ok(
        (
            ...
            type_engine.insert(TypeInfo::Unknown), // TODO: change this to the `Never` type when
                                                   // available
        ),
        warnings,
        errors,
...
fn type_check_smo(
    ...
    let mut ctx = ctx.by_ref().with_type_annotation(
        type_argument
            .clone()
            .map_or(type_engine.insert(TypeInfo::Unknown), |ta| ta.type_id),
    );
    let data = check!(
        ty::TyExpression::type_check(ctx.by_ref(), arguments[1].clone()),
        return err(warnings, errors),
    ...
    ok(
        (
            ...
            type_engine.insert(TypeInfo::Tuple(vec![])),
        ),
        warnings,
        errors,
    )
