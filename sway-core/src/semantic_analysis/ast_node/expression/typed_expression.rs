...
impl ty::TyExpression {
    ...
    pub(crate) fn type_check(mut ctx: TypeCheckContext, expr: Expression) -> CompileResult<Self> {
        ...
        let res = match expr.kind {
            ...
            ExpressionKind::LazyOperator(LazyOperatorExpression { op, lhs, rhs }) => {
                let ctx = ctx
                    .by_ref()
                    .with_type_annotation(type_engine.insert(TypeInfo::Boolean));
            ...
            ExpressionKind::Break => {
                let expr = ty::TyExpression {
                    ...
                    return_type: type_engine.insert(TypeInfo::Unknown),
                    span,
                };
                ok(expr, vec![], vec![])
            }
    ...
    fn type_check_literal(
        ...
        let id = type_engine.insert(return_type);
        let exp = ty::TyExpression {
    ...
    fn type_check_code_block(
        ...
        let (typed_block, block_return_type) = check!(
            ...
            (
                ...
                type_engine.insert(TypeInfo::Tuple(Vec::new()))
            ),
            warnings,
            errors
        );
    ...
    fn type_check_asm_expression(
        ...
        let return_type = check!(
            ctx.resolve_type(
                type_engine.insert(asm.return_type.clone()),
                &asm_span,
                EnforceTypeArguments::No,
                None
            ),
    ...
    fn type_check_tuple(
        ...
        for (i, field) in fields.into_iter().enumerate() {
            let field_type = field_type_opt
                ...
                .unwrap_or_else(|| {
                    let initial_type_id = type_engine.insert(TypeInfo::Unknown);
                    TypeArgument {
        ...
        let exp = ty::TyExpression {
            ...
            return_type: ctx.type_engine.insert(TypeInfo::Tuple(typed_field_types)),
            span,
        };
        ok(exp, warnings, errors)
    }
    ...
    fn type_check_abi_cast(
        ...
        let address_expr = {
            let ctx = ctx
                ...
                .with_type_annotation(type_engine.insert(TypeInfo::B256));
            check!(
            ...
            ty::TyDeclaration::VariableDeclaration(ref decl) => {
                ...
                match abi_name {
                    ...
                    AbiName::Deferred => {
                        return ok(
                            ty::TyExpression {
                                return_type: type_engine.insert(TypeInfo::ContractCaller {
        ...
        let return_type = type_engine.insert(TypeInfo::ContractCaller {
        ...
        if contents.is_empty() {
            ...
            return ok(
                ty::TyExpression {
                    ...
                    return_type: type_engine.insert(TypeInfo::Array(
                        TypeArgument {
    ...
    fn type_check_array_index(
        ...
        if let TypeInfo::Array(elem_type, _) = type_engine.get(prefix_te.return_type) {
            ...
            let ctx = ctx
                .with_help_text("")
                .with_type_annotation(type_engine.insert(type_info_u64));
            let index_te = check!(
    ...
    fn type_check_while_loop(
        ...
        let unit_ty = type_engine.insert(TypeInfo::Tuple(Vec::new()));
            ...
            ReassignmentTarget::VariableExpression(var) => {
                ...
                ok(
                    ty::TyExpression {
                        ...
                        return_type: type_engine.insert(TypeInfo::Tuple(Vec::new())),
                        span,
                    },
                    warnings,
                    errors,
        ...
        let (val, new_integer_type) = match lit {
            Literal::Numeric(num) => match type_engine.get(new_type) {
                ...
                TypeInfo::Numeric => (
                    ...
                    type_engine.insert(TypeInfo::UnsignedInteger(IntegerBits::SixtyFour)),
                ),
...
mod tests {
    ...
    fn do_type_check_for_boolx2(expr: Expression) -> CompileResult<ty::TyExpression> {
        ...
        do_type_check(
            Engines::new(&type_engine, &decl_engine),
            expr,
            type_engine.insert(TypeInfo::Array(
                TypeArgument {
                    type_id: type_engine.insert(TypeInfo::Boolean),
                    span: Span::dummy(),
                    call_path_tree: None,
                    initial_type_id: type_engine.insert(TypeInfo::Boolean),
                },
