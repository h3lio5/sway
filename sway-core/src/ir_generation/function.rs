...
impl<'eng> FnCompiler<'eng> {
    ...
    fn compile_contract_call(
        ...
        let ra_struct_val = self.current_block.ins(context).load(ra_struct_ptr_val);
    ...
    fn compile_fn_call(
        ...
        for ((_, expr), param) in ast_args.iter().zip(callee.parameters.iter()) {
            ...
            let arg = if param.is_reference && param.is_mutable {
                self.compile_ptr_expression(context, md_mgr, expr)
            } else {
                            ...
                            } = self.type_engine.get(*cur_type_id)
                            {
                                ...
                                Some(
                                    struct_fields
                                        ...
                                        .map(|(idx, field)| {
                                            (idx as u64, field.type_argument.type_id)
                                        })
                                        .ok_or_else(|| {
                                            CompileError::InternalOwned(
                                                format!(
